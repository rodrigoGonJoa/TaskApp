...
class GetByIdCategoryUseCase @Inject constructor(
    ...
    suspend operator fun invoke(categoryId: Long?): Result<Category, ErrorCategory> {
        if (categoryId == null) {
            logger.warn {"✘ Error: The task id is null."}
            return Result.Error(ErrorCategory.Domain.NULL_CATEGORY_ID)
        }
        return when (val result = categoryRepository.getById(categoryId)) {
            ...
            is Result.Success -> {
                if (result.data == null) {
                    ...
                    Result.Error(ErrorCategory.Domain.NULL_CATEGORY)
                }
